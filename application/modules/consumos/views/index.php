...
<!-- Main content -->
 ...
 <a href="<?php echo base_url('consumos/new_consumo'); ?> " class="btn btn-primary btn-xs" ><i class="glyphicon glyphicon-plus"></i> Nuevo Consumo</a>
...
validate();
    ...
    table = $('#tblConsumos').DataTable({
      "language":
        ...
        "ajax": {
            "url": "<?php echo site_url('consumos/ajax_list')?>",
            "type": "POST"
        },
...
function edit_customer(id)
{
    ...
    $.ajax({
       ...
       url : "http://localhost:81/sami/customers/ajax_edit/"+id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
  validate();
    ...
    var url;
    if(save_method == 'add') {
        url = "http://localhost:81/sami/customers/ajax_add/";
    } else {
        url = "http://localhost:81/sami/customers/ajax_update/";
    }
...
function delete_customer(id)
{
    if(confirm('Esta seguro de borrar esta información?'))
    {
        ...
        $.ajax({
            url : "http://localhost:81/sami/customers/ajax_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: "http://localhost:81/ronaisa/customer/ajax_bulk_delete_customer",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
...
function validate(){
  ...
  $('#msgbx_err').hide();
     $('#nit').blur(function(){
        ...
        if(email_val){
            $('#msgbx_err').show();
            $.post("http://localhost:81/sami/customers/nit_check_customer", {
                nit: email_val
            }, function(response){
                $('#loading').hide();
