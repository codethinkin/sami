      ...
      <div class="col-xs-12">
        <div class="box box-primary">
          <div class="box-header with-border">
            <h3 class="box-title">Tablero | Frecuencias  </h3>
            <div class="box-tools">

 <button class="btn btn-primary btn-xs" onclick="add_frequency()"><i class="glyphicon glyphicon-plus"></i> Nueva Frecuencia</button>
          ...
          <!-- /.box-header -->
          <div class="box-body">
            <table id="tblfrequency" class="table table-striped table-bordered table-responsive" cellspacing="0" width="100%">
            <thead>
                <tr>
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group">
                          <label class="control-label col-md-3">Nombre</label>
                            <div class="col-md-9">
                                <input name="nameFrequency" id="nameFrequency" placeholder="" class="form-control input-sm input-sm" type="text" required autofocus>
...
validate();
    ...
    table = $('#tblfrequency').DataTable({
      "language":
        ...
        "ajax": {
            "url": "<?php echo site_url('frequency/ajax_list')?>",
            "type": "POST"
        },
...
function add_frequency(){
    ...
    $('.modal-title').text('Registro Nueva Frecuencia'); // Set Title to Bootstrap modal title
}
...
function edit_frequency(id)
{
    ...
    $.ajax({
         //url: "<?php echo site_url('frequency/ajax_edit')?>"/+id,
       url : currentLocation + "/frequency/ajax_edit/"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="id"]').val(data.id);
            $('[name="nameFrequency"]').val(data.nameFrequency);
...
function save()
{
  validate();
    ...
    var url;
    if(save_method == 'add') {
        url = currentLocation + "/frequency/ajax_add/";
    } else {
        url = currentLocation + "/frequency/ajax_update/";
    }
...
function delete_frequency(id)
{
    if(confirm('Esta seguro de borrar esta información?'))
    {
        // ajax delete data to database
        $.ajax({
            url : currentLocation + "/frequency/ajax_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: currentLocation + "/frequency/ajax_bulk_delete_frequency",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
...
$('#msgbx_err').hide();
$('#nameFrequency').blur(function() {
var url= currentLocation + "/frequency/chk_usr";
          var name = $("#nameFrequency").val();
                   $.post(url, {code: name}, function(d) {
                       if (d == 1)
                       {
                          //  alertify.alert();
                       alertify.alert('Error!', 'La Frecuencia ya se encuentra registrada', function(){ alertify.success('Ok'); });
                          // $('#msgbx_err').show();
                          // $('#code').focus();
                       }
                       else
...
function detail_frequency(id)
{
  ...
  $.ajax({
      ...
      success: function(data)
      {
          ...
          $('[name="nameFrequency"]').val(data.nameFrequency);
          $('[name="state"]').val(data.state);
         $('.modal-title').text('Movimientos de Frecuencia en Mantenimiento'); // Set Title to Bootstrap modal title
