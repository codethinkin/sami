      ...
      <div class="col-xs-12">
        <div class="box box-primary">
          <div class="box-header with-border">
            <h3 class="box-title">Tablero | Actividades  </h3>
            <div class="box-tools">

 <button class="btn btn-primary btn-xs" onclick="add_activity()"><i class="glyphicon glyphicon-plus"></i> Nueva Actividad</button>
          ...
          <!-- /.box-header -->
          <div class="box-body">
            <table id="tblactivity" class="table table-striped table-bordered table-responsive" cellspacing="0" width="100%">
            <thead>
                <tr>
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group">
                          <label class="control-label col-md-3">Nombre</label>
                            <div class="col-md-9">
                                <input name="nameActivity" id="nameActivity" placeholder="" class="form-control input-sm input-sm" type="text" required autofocus>
...
validate();
    ...
    table = $('#tblactivity').DataTable({
      "language":
        ...
        "ajax": {
            "url": "<?php echo site_url('activity/ajax_list')?>",
            "type": "POST"
        },
...
function add_activity(){
    ...
    $('.modal-title').text('Registro Nueva Actividad'); // Set Title to Bootstrap modal title
}
...
function edit_activity(id)
{
    ...
    $.ajax({
       ...
       url : currentLocation + "/activity/ajax_edit/"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="id"]').val(data.id);
            $('[name="nameActivity"]').val(data.nameActivity);
            $('[name="state"]').val(data.state);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Editar Actividad'); // Set title to Bootstrap modal title
        },
...
function save()
{
  validate();
    ...
    var url;
    if(save_method == 'add') {
        url = currentLocation + "/activity/ajax_add/";
    } else {
        url = currentLocation + "/activity/ajax_update/";
    }
...
function delete_activity(id)
{
    if(confirm('Esta seguro de borrar esta información?'))
    {
        ...
        $.ajax({
            url : currentLocation + "/activity/ajax_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: currentLocation + "/activity/ajax_bulk_delete_activity",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
...
$('#msgbx_err').hide();
$('#name').blur(function() {
var url= currentLocation + "/activity/chk_usr";
...
function detail_activity(id)
{
  ...
  $.ajax({
      ...
      success: function(data)
      {
          ...
          $('[name="nameActivity"]').val(data.nameActivity);
          $('[name="state"]').val(data.state);
         $('.modal-title').text('Movimientos del activityos'); // Set Title to Bootstrap modal title
