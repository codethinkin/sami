      ...
      <div class="col-xs-12">
        <div class="box box-primary">
          <div class="box-header with-border">
            <h3 class="box-title">Tablero | Ordenes de Actividades  </h3>
            <div class="box-tools">

 <button class="btn btn-primary btn-xs" onclick="add_category()"><i class="glyphicon glyphicon-plus"></i> Nueva Orden</button>
          ...
          <!-- /.box-header -->
          <div class="box-body">
            <table id="tblcategory" class="table table-striped table-bordered table-responsive" cellspacing="0" width="100%">
            <thead>
                <tr>
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group">
                          <label class="control-label col-md-3">Nombre</label>
                            <div class="col-md-9">
                                <input name="name" id="name" placeholder="" class="form-control input-sm input-sm" type="text" required autofocus>
...
function edit_category(id)
{
    ...
    $.ajax({
       ...
       url : "http://localhost:81/sami/category/ajax_edit/"+id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
  validate();
    ...
    var url;
    if(save_method == 'add') {
        url = "http://localhost:81/sami/category/ajax_add/";
    } else {
        url = "http://localhost:81/sami/category/ajax_update/";
    }
...
function delete_category(id)
{
    if(confirm('Esta seguro de borrar esta información?'))
    {
        ...
        $.ajax({
            url : "http://localhost:81/sami/category/ajax_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: "http://localhost:81/ronaisa/category/ajax_bulk_delete_category",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
...
function validate(){
  ...
  $('#msgbx_err').hide();
     $('#name').blur(function(){
        ...
        if(email_val){
            $('#msgbx_err').show();
            $.post("http://localhost:81/sami/category/name_check_category", {
                name: email_val
            }, function(response){
                $('#loading').hide();
...
function detail_category(id)
{
  ...
  $.ajax({
      ...
      success: function(data)
      {
          ...
          $('[name="name"]').val(data.name);
          $('[name="state"]').val(data.state);
         $('.modal-title').text('Movimientos del categoryos'); // Set Title to Bootstrap modal title
