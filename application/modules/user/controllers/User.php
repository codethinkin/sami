...
is_login();
		...
		if(setting_all('register_allowed')==1){
			if($this->input->post()) {
				$this->add_edit();
				$this->session->set_flashdata('messagePr', 'Registrado exitosamente..');
			} else {
				$this->load->view('include/script');
			...
			$this->session->set_flashdata('messagePr', 'Registro no permitido..');
			redirect( base_url().'user/login', 'refresh');
		}
    }
...
$this->load->library('Recaptcha');
      // Catch the user's answer
		...
		if(empty($return)) {
			$this->session->set_flashdata('messagePr', 'Datos no validos');
            redirect( base_url().'user/login', 'refresh');
        } else {
			if($return == 'not_varified') {
				$this->session->set_flashdata('messagePr', 'Esta cuenta no esta verificada, pongase en contacto con el administrador..');
                redirect( base_url().'user/login', 'refresh');
			} if($response['success']) {
...
Para restablecer tu contraseña, se ha enviado un enlace a tu correo electrónico');
                    redirect( base_url().'user/login','refresh');
                }
            } else {
    ...
    public function reset_password(){
        ...
        if($return){
        	$this->session->set_flashdata('messagePr', 'Contraseña fue cambiada con éxito..');
            redirect( base_url().'user/login', 'refresh');
        } else {
        	$this->session->set_flashdata('messagePr', '
