      ...
      <div class="col-xs-12">
        <div class="box box-primary">
          <div class="box-header with-border">
            <h3 class="box-title">Tablero | Proveedores Activos </h3>
            <div class="box-tools">

 <button class="btn btn-primary btn-xs" onclick="add_provider()"><i class="glyphicon glyphicon-plus"></i> Nuevo Proveedor</button>
          ...
          <!-- /.box-header -->
          <div class="box-body">
            <table id="tblprovider" class="table table-striped table-bordered table-responsive" cellspacing="0" width="100%">
            <thead>
                <tr>
                    ...
                    <div class="form-body">
                      ...
                      <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <input name="email" id="email" placeholder="" class="form-control input-sm" type="numeric">
                                <span class="help-block"></span>
                          </div>
                        </div>
                        <div class="form-group">
...
validate();
    ...
    table = $('#tblprovider').DataTable({
      "language":
        ...
        "ajax": {
            "url": "<?php echo site_url('provider/ajax_list')?>",
            "type": "POST"
        },
...
function add_provider(){
    ...
    $('.modal-title').text('Registro Nuevo Proveedor'); // Set Title to Bootstrap modal title
}
...
function edit_provider(id)
{
    ...
    $.ajax({
         //url: "<?php echo site_url('provider/ajax_edit')?>"/+id,
       url : "http://localhost:81/sami/provider/ajax_edit/"+id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
  validate();
    ...
    var url;
    if(save_method == 'add') {
        url = "http://localhost:81/sami/provider/ajax_add/";
    } else {
        url = "http://localhost:81/sami/provider/ajax_update/";
    }
...
function delete_provider(id)
{
    if(confirm('Esta seguro de borrar esta información?'))
    {
        ...
        $.ajax({
            url : "http://localhost:81/sami/provider/ajax_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: "http://localhost:81/ronaisa/provider/ajax_bulk_delete_provider",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
...
function validate(){
  ...
  $('#msgbx_err').hide();
     $('#nit').blur(function(){
        ...
        if(email_val){
            $('#msgbx_err').show();
            $.post("http://localhost:81/sami/provider/nit_check_provider", {
                nit: email_val
            }, function(response){
                $('#loading').hide();
...
function detail_provider(id)
{
  ...
  $.ajax({
      ...
      success: function(data)
      {
          ...
          $('[name="address"]').val(data.address);
          $('[name="email"]').val(data.email);
         $('.modal-title').text('Movimientos del provideros'); // Set Title to Bootstrap modal title
