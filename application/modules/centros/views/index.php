...
<!-- Main content -->
 ...
 <button class="btn btn-primary btn-xs" onclick="add_centros()"><i class="glyphicon glyphicon-plus"></i> Nuevo Centro de Costos</button>
          ...
          <!-- /.box-header -->
          <div class="box-body">
            <table id="tblcentros" class="table table-striped table-bordered table-responsive" cellspacing="0" width="100%">
            <thead>
                <tr>
                ...
                <input type="hidden" value="" name="id"/>
                    <div class="form-body">
                        ...
                        <label class="control-label col-md-3">Centro de Costo</label>
                          <div class="col-md-9">
                              <input name="centocosto" id="centocosto" placeholder="" class="form-control input-sm input-sm" type="text" required>
                              <span class="help-block"></span><span id="name_result"></span>
                          </div>
                      </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <select name="estatus" id="state" class="form-control input-sm">
...
validate();
    ...
    table = $('#tblcentros').DataTable({
      "language":
        ...
        "ajax": {
            "url": "<?php echo site_url('centros/ajax_list')?>",
            "type": "POST"
        },
...
function edit_category(id)
{
    ...
    $.ajax({
       ...
       url : currentLocation + "/centros/ajax_edit/"+id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
  validate();
    ...
    var url;
    if(save_method == 'add') {
        url = currentLocation + "/centros/ajax_add/";
    } else {
        url = currentLocation + "/centros/ajax_update/";
    }
...
function delete_category(id)
{
    if(confirm('Esta seguro de borrar esta información?'))
    {
        ...
        $.ajax({
            url : currentLocation + "/centros/ajax_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: currentLocation + "/centros/ajax_bulk_delete_category",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
...
$('#msgbx_err').hide();
$('#name').blur(function() {
var url= currentLocation + "/centros/chk_usr";
          var name = $("#name").val();
