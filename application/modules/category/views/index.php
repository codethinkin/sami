      ...
      <div class="col-xs-12">
        <div class="box box-primary">
          <div class="box-header with-border">
            <h3 class="box-title">Tablero | Categorias  </h3>
            <div class="box-tools">

 <button class="btn btn-primary btn-xs" onclick="add_category()"><i class="glyphicon glyphicon-plus"></i> Nueva Categoria</button>
...
function edit_category(id)
{
    ...
    $.ajax({
       ...
       url : currentLocation + "/category/ajax_edit/"+id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
  validate();
    ...
    var url;
    if(save_method == 'add') {
        url = currentLocation + "/category/ajax_add/";
    } else {
        url = currentLocation + "/category/ajax_update/";
    }
...
function delete_category(id)
{
    if(confirm('Esta seguro de borrar esta información?'))
    {
        ...
        $.ajax({
            url : currentLocation + "/category/ajax_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: currentLocation + "/category/ajax_bulk_delete_category",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
...
$('#msgbx_err').hide();
$('#name').blur(function() {
var url= currentLocation + "/category/chk_usr";
          var name = $("#name").val();
