...
<!-- Main content -->
 ...
 <button class="btn btn-primary btn-xs" onclick="add_activityfrequency()"><i class="glyphicon glyphicon-plus"></i> Nueva Sub-Categoria</button>
          ...
          <!-- /.box-header -->
          <div class="box-body">
            <table id="tblactivityfrequency" class="table table-striped table-bordered table-responsive" cellspacing="0" width="100%">
            <thead>
                <tr>
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group">
                          <label class="control-label col-md-3">Frecuencia</label>
                            <div class="col-md-9">
                              <select name="frequency" class="form-control input-sm" data-validate="required" id="id"
                        ...
                        <label class="control-label col-md-3">Actividad</label>
                          <div class="col-md-9">
                              <input name="activityFrequency" id="activityFrequency" placeholder="" class="form-control input-sm input-sm" type="text" required>
                              <span class="help-block"></span><span id="name_result"></span>
                          </div>
                      </div>
...
validate();
    ...
    //datatables
    table = $('#tblactivityfrequency').DataTable({
      "language":
                {
        ...
        "ajax": {
            "url": "<?php echo site_url('activityfrequency/ajax_list')?>",
            "type": "POST"
        },
...
function add_activityfrequency(){
    save_method = 'add';
    $("#state").hide();
    $("#lblState").hide();
...
function edit_activityfrequency(id)
{
    save_method = 'update';
    $("#state").show();
    $("#lblState").show();
    ...
    $.ajax({
         //url: "<?php echo site_url('activityfrequency/ajax_edit')?>"/+id,

       url : currentLocation + "/activityfrequency/ajax_edit/"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="activityfrequency"]').val(data.activityfrequency);
            $('[name="state"]').val(data.state);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Editar Actividad Por Frecuencia'); // Set title to Bootstrap modal title
        },
...
function save()
{
  //validate();
    ...
    var url;
    if(save_method == 'add') {
        url = currentLocation + "/activityfrequency/ajax_add/";
    } else {
        url = currentLocation + "/activityfrequency/ajax_update/";
    }
...
function delete_activityfrequency(id)
{
    if(confirm('Esta seguro de borrar esta información?'))
    {
        // ajax delete data to database
        $.ajax({
            url : currentLocation + "/activityfrequency/ajax_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: currentLocation + "/activityfrequency/ajax_bulk_delete_activityfrequency",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
...
$('#msgbx_err').hide();
$('#name').blur(function() {
var url= currentLocation + "/activityfrequency/chk_usr";
          var name = $("#name").val();
...
function detail_activityfrequency(id)
{
  $('#form')[0].reset(); // reset form on modals
  $('.form-group').removeClass('has-error'); // clear error class
  $('.help-block').empty(); // clear error string
